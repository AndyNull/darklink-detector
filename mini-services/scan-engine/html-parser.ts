  ...
  for (const { tag, attr } of tagAttrPairs) {
    $(tag).each((_, el) => {
      ...
      if (['img', 'source'].includes(tag) && attr === 'src') {
        ...
        // Also check data-src for lazy-loaded images (common on sites like chinaunicom.com)
...
function extractRegexScan(html: string, baseUrl: string): RawUrlEntry[] {
  ...
  const addEntry = (url: string, source: string, context?: string) => {
    const resolvedUrl = resolveUrl(url, baseUrl);
    if (!resolvedUrl || seen.has(resolvedUrl)) return;
    // Validate that the resolved URL actually produces a valid domain
    const domain = extractDomain(resolvedUrl);
    if (!domain) return;
    seen.add(resolvedUrl);
    entries.push({
  ...
  // 8a. http:// and https:// URLs — require at least a domain-like part after protocol
  // Avoid matching URLs inside CSS url() declarations (those are handled separately)
  for (const m of html.matchAll(/https?:\/\/[^\s"'<>\\)}\]]+/g)) {
    let url = m[0].replace(/[.,;:!?)\]}>]+$/, ''); // strip trailing punctuation
    // Skip very short URLs that are likely false positives
    if (url.length < 12) continue; // http://a.b is minimum valid
    // Skip URLs that are clearly inside CSS/JS code constructs
    if (url.includes('url(') || url.includes('url (')) continue;
    addEntry(url, 'regex-scan', url);
  }

  // 8b. Protocol-relative URLs with a domain
  // Must have at least one dot in the hostname part to be valid
  for (const m of html.matchAll(/\/\/([^\s"'<>\\)}\]]+\.[a-z]{2,}[^\s"'<>\\)}\]]*)/gi)) {
    let url = m[0].replace(/[.,;:!?)\]}>]+$/, '');
    // Ensure it looks like a real URL (has a path or at least a TLD)
    if (url.length <= 5) continue;
    // Skip if this is inside a URL we already found (e.g. src="//...")
    // The protocol-relative URL must have a proper domain structure
    const hostPart = url.slice(2).split('/')[0];
    if (!hostPart.includes('.')) continue;
    addEntry(url, 'regex-scan', url);
  }

  // 8c. Domain patterns without protocol
  // Be more strict: require the match to be preceded by a non-alphanumeric character
  // and the domain must have at least 2 labels (e.g. "example.com" not just "com")
  ...
  for (const m of html.matchAll(domainRegex)) {
    ...
    // Skip if it's just a TLD-like extension in a filename (e.g. "script.js")
    // Only add if it looks like a standalone domain reference
    if (/^[\d.]+$/.test(domain)) continue;
    // Skip common file extension patterns
    if (/^\w+\.(js|css|html?|php|json|xml|txt|png|jpe?g|gif|svg|ico|woff2?|ttf|eot|mp[34]|pdf|zip|rar)$/i.test(domain)) continue;
...
export function extractImageUrls(html: string, baseUrl: string): string[] {
  ...
  // 1. <img> tags – src, srcset, data-src (lazy loading), data-lazy-src, data-original, data-srcset
  $('img').each((_, el) => {
    ...
    // Handle srcset and data-srcset (lazy loading variant)
  ...
  // 2. <picture> / <source> with srcset/src/data-src/data-srcset (lazy loading)
  ...
  // 4. SVG <image> and <use> elements (including data-href for lazy SVG loading)
  $('svg image, svg use').each((_, el) => {
    for (const attr of ['href', 'xlink:href', 'data-href']) {
      const val = $(el).attr(attr);
      if (val) {
        const resolved = resolveImageUrl(val, baseUrl);
        if (resolved) addUrl(resolved);
