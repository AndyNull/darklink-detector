...
// ──── Threat Intel Result (for popover, uses single query) ────
export const ThreatIntelResult = React.memo(function ThreatIntelResult({ type, value }: { type: 'domain' | 'ip'; value: string }) {
  ...
  const handleAddToMalicious = async () => {
    ...
    setAdding(true);
    try {
      const token = typeof window !== 'undefined' ? localStorage.getItem('darklink-auth-token') : null;
