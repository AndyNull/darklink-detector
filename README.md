...
- **扫描指纹多样化** — 每个并发扫描线程分配独立的浏览器指纹（UA/Accept-Language/视口），12 种指纹池轮换，同一 URL 的所有请求保持一致以模拟单一浏览器会话
- **审计日志关联性修复** — 为日志条目新增 `entityType`/`entityId`/`metadata` 字段，支持与扫描任务、情报源等实体的结构化关联
- **日志详情展开** — 审计日志面板支持展开查看结构化元数据，实体类型显示中文标签
- **下载迁移至 GitHub Releases** — 项目打包不再放在 public/ 目录，改用 GitHub Releases 分发
...
- ✅ 允许商业使用
- ⚠️ 修改后的代码必须以相同许可证开源
- ⚠️ 通过网络提供服务时，必须向用户提供源代码
- 🚫 不允许闭源分发或闭源网络服务

**免责声明：** 本软件按"原样"提供，不作任何明示或暗示的保证。对于因使用本软件而产生的任何直接、间接、偶然、特殊或后果性损害，作者不承担任何责任。安全扫描工具可能产生误报或漏报，使用者应自行验证检测结果。

© 2025-2026 DarkLink Detector Contributors
